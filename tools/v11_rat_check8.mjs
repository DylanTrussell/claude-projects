...
// stand exactly 1.4 tiles from the enemy on the open side, force it awake
...
for (let i = 0; i < 60; i++) {
  ...
  if (e.st === 'lunge' && !gotLunge) {
    gotLunge = true;
    await page.screenshot({ path: '/tmp/v11_rat_lunge_clean.png' });
    console.log(i, 'LUNGE CAPTURED', e.x.toFixed(2), e.y.toFixed(2));
  }
  console.log(i, e.st, e.x.toFixed(2), e.y.toFixed(2));
  if (e.st === 'recover' || e.dead) break;
