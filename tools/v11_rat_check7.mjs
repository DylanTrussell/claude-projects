...
// Stand back 2.5 tiles, force the enemy awake (dev hook), look at it, screenshot the walk cycle.
await page.evaluate(([ex, ey]) => window.__AMtp(ex + 2.0, ey, undefined), [e0.x, e0.y]);
...
for (let i = 0; i < 14; i++) {
  ...
  await page.screenshot({ path: `/tmp/v11_rat_w${i}.png` });
  console.log(i, e.st, e.x.toFixed(2), e.y.toFixed(2), 'dist=', Math.hypot(e.x - (e0.x - 2.5), e.y - e0.y).toFixed(2));
  if (e.st === 'lunge') break;
