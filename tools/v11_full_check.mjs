...
// --- pause menu (do this now, mid-tunnel, then resume) ---
await page.keyboard.press('Escape');
await page.waitForTimeout(300);
await page.screenshot({ path: '/tmp/v11_pause_menu.png' });
await page.keyboard.press('Escape');
await page.waitForTimeout(300);
...
for (let i = 0; i < 20; i++) {
  ...
  const st = await page.evaluate(() => window.__AM().tunnel?.script);
  if (st?.phase === 'rip') { console.log('entered rip at iter', i); break; }
...
for (let i = 0; i < 40; i++) {
  ...
  if (!gotEarly && st.t < 480) {
    await page.screenshot({ path: '/tmp/v11_rip_mid.png' });
    console.log('saved rip_mid at t=', st.t);
    gotEarly = true;
  }
  if (!gotLate && st.t >= 520 && st.t < 890) {
    await page.screenshot({ path: '/tmp/v11_rip_aftermath.png' });
